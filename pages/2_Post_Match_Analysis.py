...
# Page title
st.markdown("# Olympic's Previous Matches")
st.sidebar.header("Post-match Analysis")
...
for i in range(len(fixtures)):
    ...
    if (fixtures["Data availability"][i] == "Y"):
        availableMatch = "Round " + str(int(fixtures["Round"][i])) + " - " + fixtures["Team"][i] + " (" + \
...
match_round = matchOption.split(" ")[1]
home_team = ''
away_team = ''
...
if ('Brisbane City' in matchOption):
    ...
    if ('Home' in matchOption):
        ...
        away_team = 'BOL'
        home_team = 'PEN'
elif ('Redlands United' in matchOption):
    if ('Home' in matchOption):
        home_team = 'BOL'
        away_team = 'RED'
    elif ('Away' in matchOption):
        away_team = 'BOL'
        home_team = 'RED'
elif ('Rochedale Rovers' in matchOption):
    if ('Home' in matchOption):
        home_team = 'BOL'
        away_team = 'ROC'
    elif ('Away' in matchOption):
...
directory = 'Opta stats/2023 data/'
...
with st.spinner("While waiting, remember to stay hydrated!"):
    ...
    if (vizOption == 'xG timeline') or (vizOption == 'Shot map'):
        ...
        isHomeTeam = False
        # Open the json file, copy its data, and then immediately close the json 
        # Print an error message if the file does not exist
        try:
        ...
        for index, event in enumerate(event):
            ...
            if (event['contestantId'] == homeTeamId):
                ...
                # Get the typeId and determine the team who made the shot
                xGoalEvent['shotType'] = event['typeId']
                xGoalEvent['shootingTeam'] = 'home'
                # Get the x and y coordinates of the shot
