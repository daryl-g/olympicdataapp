...
sys.path.append('../olympicdataapp')
from data_processing import totalData
...
nplqTeams = ['Brisbane City', 'Brisbane Roar Youth', 'Eastern Suburbs', 'Gold Coast Knights',
            'Gold Coast United', 'Lions', 'Moreton Bay United', 'Olympic',
            'Peninsula Power', 'Redlands United', 'Rochedale Rovers', 'Sunshine Coast Wanderers']
...
for i in range(len(nplqFixtures)):
    ...
    if (nplqFixtures["Data availability"][i] == "Y"):
        ...
        if (nplqFixtures["Round"][i] == "SF"):
            ...
            xGoalFile = str(matchRound) + '_' + homeTeamCode + "_" + awayTeamCode + "_xgoal_stats.json"
            statsFile = str(matchRound) + '_' + homeTeamCode + "_" + awayTeamCode + "_stats.json"
            eventsFile = str(matchRound) + '_' + homeTeamCode + "_" + awayTeamCode + "_events.json"
        ...
        if (homeTeamCode == "RED") or (awayTeamCode == "RED"):

            xgoalRedlands.append(xGoalFile)
            statsRedlands.append(statsFile)
            eventsRedlands.append(eventsFile)

        if (homeTeamCode == "ROC") or (awayTeamCode == "ROC"):

            xgoalRochedale.append(xGoalFile)
            statsRochedale.append(statsFile)
            eventsRochedale.append(eventsFile)
...
with st.spinner("This app is trying its hardest to bring you the best visualisation!"):
    ...
    BrisbaneCity = totalData(teamIdBrisbaneCity, directoryBrisbaneCity, xgoalBrisbaneCity, statsBrisbaneCity, eventsBrisbaneCity)
    RoarYouth = totalData(teamIdRoarYouth, directoryRoarYouth, xgoalRoarYouth, statsRoarYouth, eventsRoarYouth)
    EastsSuburbs = totalData(teamIdEastsSuburbs, directoryEasternSuburbs, xgoalEastsSuburbs, statsEastsSuburbs, eventsEastsSuburbs)
    GCKnights = totalData(teamIdGCKnights, directoryGCKnights, xgoalGCKnights, statsGCKnights, eventsGCKnights)
    GCUnited = totalData(teamIdGCUnited, directoryGCUnited, xgoalGCUnited, statsGCUnited, eventsGCUnited)
    Lions = totalData(teamIdLions, directoryLions, xgoalLions, statsLions, eventsLions)
    MoretonBayUtd = totalData(teamIdMoretonBayUtd, directoryMoretonBayUtd, xgoalMoretonBayUtd, statsMoretonBayUtd, eventsMoretonBayUtd)
    Olympic = totalData(teamIdOlympic, directoryOlympic, xgoalOlympic, statsOlympic, eventsOlympic)
    PenPower = totalData(teamIdPenPower, directoryPenPower, xgoalPenPower, statsPenPower, eventsPenPower)
    Redlands = totalData(teamIdRedlands, directoryRedlands, xgoalRedlands, statsRedlands, eventsRedlands)
    Rochedale = totalData(teamIdRochedale, directoryRochedale, xgoalRochedale, statsRochedale, eventsRochedale)
    SCWanderers = totalData(teamIdSCWanderers, directorySCWanderers, xgoalSCWanderers, statsSCWanderers, eventsSCWanderers)

    params = list(Olympic.keys())

    teamStats = [BrisbaneCity, RoarYouth, EastsSuburbs, GCKnights, GCUnited, Lions,
                MoretonBayUtd, Olympic, PenPower, Redlands, Rochedale, SCWanderers]
    ...
    if (teamOption == 'Brisbane City'): 
        ...
        teamA_edgecolor = 'red'
    elif (teamOption == 'Redlands United'): 
        teamA = Redlands
        teamA_facecolor = 'red'
        teamA_edgecolor = 'black'
    elif (teamOption == 'Rochedale Rovers'): 
        teamA = Redlands
        teamA_facecolor = '#f8d018'
        teamA_edgecolor = '#009b00'
    ...
    if (vizOption == 'Radar plot'):
        ...
        if (yesComparison == True):
            ...
            if (secondTeamOption == 'Brisbane City'): 
                ...
                teamB_edgecolor = 'red'
            elif (secondTeamOption == 'Redlands United'): 
                teamB = Redlands
                teamB_facecolor = 'red'
                teamB_edgecolor = 'black'
            elif (secondTeamOption == 'Rochedale Rovers'): 
                teamB = Redlands
                teamB_facecolor = '#f8d018'
                teamB_edgecolor = '#009b00'
        ...
        redlands = plt.imread("Logos/Redlands.png")
        roar_youth = plt.imread("Logos/Brisbane Roar.png")
        rochedale = plt.imread("Logos/Rochedale.png")
        ...
        with col4:
            ...
            if (teamOption == 'Brisbane City'): 
                ...
                team_xvalue = Redlands[scatter_params[0]]
                team_yvalue = scatter_yvalues[9]
            elif (teamOption == 'Rochedale Rovers'): 
                team_xvalue = rochedale[scatter_params[0]]
        ...
        if (teamOption == 'Brisbane City'): 
            ...
            xgoal_list = xgoalRedlands
            events_list = eventsRedlands
        elif (teamOption == 'Rochedale Rovers'): 
            directory = directoryRochedale
            teamId = teamIdRochedale
            xgoal_list = xgoalRochedale
            events_list = eventsRochedale
        elif (teamOption == 'Sunshine Coast Wanderers'): 
        ...
        if (vizOption == 'Shot map'):
            ...
            competition_info = "Queensland NPL - 2023 season"
            ...
            if (zonalOption == 'Defensive actions') or (zonalOption == 'Possession won') or (zonalOption == 'Possession lost'):
                ...
                # heatmap and labels
                bin_statistic = pitch.bin_statistic(x_coordinates, y_coordinates,
                ...
                competition_info = 'NPL Queensland - 2023 season'
